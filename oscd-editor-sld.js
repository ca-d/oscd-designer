...
import { attributes, elementPath, eqTypes, isBusBar, privType, removeNode, removeTerminal, reparentElement, sldNs, uuid, xmlnsNs, } from './util.js';
...
class OscdEditorSLD extends LitElement {
    ...
    render() {
      ...
      <nav>
        ${Array.from(this.doc.querySelectorAll(':root > Substation > VoltageLevel > Bay')).find(bay => !isBusBar(bay))
            ? eqTypes
                ...
                this.startPlacing(element);
                    ...
                    >`)
                .concat()
            : nothing}${this.doc.querySelector(':root > Substation > VoltageLevel')
            ? html `<mwc-fab
                  mini
            ...
            : nothing}${Array.from(this.doc.documentElement.children).find(c => c.tagName === 'Substation')
            ? html `<mwc-fab
                mini
        ...
        >${Array.from(this.doc.documentElement.children).find(c => c.tagName === 'Substation')
            ? html `<mwc-fab
                ...
                >`
            : nothing}${this.doc.querySelector('VoltageLevel, PowerTransformer')
      ...
      ${Array.from(this.doc.querySelectorAll(':root > Substation')).map(subs => html `<sld-editor
            .editor=${this.editor}
            .doc=${this.doc}
